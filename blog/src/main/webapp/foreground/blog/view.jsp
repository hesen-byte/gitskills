...
<script type="text/javascript">
	...
	function submitData(){
		...
		// 	alert("请填写验证码！");
			...
			$.post("${pageContext.request.contextPath}/comment/save",{'content':content,'imageCode':imageCode,'blog.id':'${blog.id}'},function(result){
				if(result.success){
			...
			<c:choose>
				...
				<c:otherwise>
					<c:forEach var="keyWord" items="${keyWords }">
						&nbsp;&nbsp;<a href="${pageContext.request.contextPath}/blog/q?q=${keyWord}" target="_blank">${keyWord }</a>&nbsp;&nbsp;
					</c:forEach>
				</c:otherwise>
			</c:choose>
		</div>
