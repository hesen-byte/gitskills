...
<script type="text/javascript">
	...
	function formatTitle(val,row){
		return "<a target='_blank' href='${pageContext.request.contextPath}/blog/articles/"+row.id+"'>"+val+"</a>"
	}
	...
	function deleteBlog(){
		...
		if(selectedRows.length==0){
		 ...
		 $.messager.confirm("系统提示","您确定要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？",function(r){
				if(r){
					$.post("${pageContext.request.contextPath}/admin/blog/delete",{ids:ids},function(result){
						if(result.success){
							if (result.exist) {
								$.messager.alert("系统提示","该博客类别下有博客，不能删除！");
							} else {
...
<table id="dg" title="博客管理" class="easyui-datagrid"
   ...
   <thead>
   	<tr>
<%--		这里对title,releaseDate,和blogType都做了一些转换，releaseDate的转换是在controller完成的--%>
